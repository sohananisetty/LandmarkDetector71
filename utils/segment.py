...
class Segmenter():
    ...
    def __init__(self , model_path = './pretrained_models/' , device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        self.model_path = os.path.join(model_path , "79999_iter.pth")
        self.n_classes = 19
        self.device = device
    ...
    def return_hair_mask(self , image):
        out = self.segment_image(image)
        hairmask = np.where(out == 17, 1, 0)  # index corresponding to hair segmentation
        return hairmask

    def return_face_mask(self,image):
        out = self.segment_image(image)
        # print(np.unique(out))
    ...
    def return_skin_mask(self , image):
        # if np.max(image)> 10 :
        #   image = image/255
    ...
    def return_full_mask(self , image):
        out = self.segment_image(image)
        # print(np.unique(out))
        hairmask = np.zeros(out.shape)
        hairmask[(out == 1)] = 1  # face
    ...
    def returnfacebbox(self , image, margin=0.05, msk_type='face', getbbox=False, padding=False):
        ### input is numpy Image

        image = np.array(image)
        if msk_type == 'face':
            mask = self.return_face_mask(image)

        elif msk_type == 'full':
            mask = self.return_full_mask(image)
