...
def crop(img, center, scale, res=[256, 256], rot=0):
    ...
    new_img = util.im_to_torch(np.array(Image.fromarray(new_img).resize((256, 256))))
    return new_img
...
def transform_preds(coords, center, scale, res):
    ...
    for p in range(coords.size(0)):
        coords[p, 0:2] = util.to_torch(transform(coords[p, 0:2], center, scale, res, 1, 0))
    return coords
